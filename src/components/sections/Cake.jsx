    ...
    return (
        <section id="cake" className=" min-h-screen flex items-center justify-center py-20">
            <div className="max-w-5xl mx-auto px-4">
                ...
                <div className="grid grid-cols-3 border w-full my-5 border-[#6b5a4c]  w-145 md:w-200 md:grid-cols-5 justify-center  ">
                    {cakes.map((cake) => {
                        return (
                            ...
                            <div key={cake.id} className="flex p-2 w-full md:p-4 flex-col justfiy-center border border-[#6b5a4c] items-center">
                                ...
                                <h1 className="font-semibold text-[8px] md:text-lg">{cake.name}</h1>
                                <p className="font-bold text-[10px] text-[#6b5a4c]">{`₦${cake.price}`}</p>
                                <button className="rounded-full mt-3 p-1 bg-[#e67e22] text-[#3e2c23] cursor-pointer text-white text-[10px] font-bold w-16 md:w-20 px-2" onClick={() => handleAddProduct(cake.name, cake.price, 1)}>Order</button>
                            </div>
