    ...
    return (
        <section id="cake" className=" min-h-screen flex items-center justify-center py-20">
            <div className="max-w-5xl mx-auto px-4">
                ...
                <div className="grid grid-cols-4 my-5 border-[#6b5a4c]  w-145 md:w-200 md:grid-cols-5 justify-center  ">
                    {meatpies.map((meatpie) => {
                        return (
                            ...
                            <div key={meatpie.id} className="flex py-4 md:p-4 flex-col justfiy-center cursor-pointer hover:translate-y-1 hover:border hover:rounded hover:shadow-lg items-center">
                                <img src={meatpie.src} className="h-35 w-30 md:h-42 md:w-40 rounded" />
                                <h1 className="font-semibold md:text-lg">{meatpie.name}</h1>
                                <p>1000</p>
                                <button className="rounded-full mt-3 p-1 bg-[#fa8072] text-[#3e2c23] cursor-pointer text-white text-[10px] font-bold w-20 px-2" onClick={() => handleAddProduct(meatpie.name, 1000, 1)}>Order</button>
                            </div>
